"""arc.analyze.dashboards.renderer
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
from arc.analyze.dashboards.aggregator import AggregatedReport
...
def render_html(report: AggregatedReport, out_path: Path) -> None:
    ...
    html = (
        ...
        "<title>ARC Analysis Dashboard</title>"
        f"{_HTML_STYLE}"
        "</head><body>"
        "<header><h1>ARC Analysis Dashboard</h1>"
        "<p>Generated by <code>arc analyze dashboard</code></p></header>"
        "<main>"
        ...
        "</main>"
        "<footer>arc analyze dashboard — plugin-agnostic framework analysis</footer>"
        "</body></html>"
    )
...
def render_markdown_summary(report: AggregatedReport, out_path: Path) -> None:
    ...
    lines = ["# ARC Analysis Dashboard — Summary", ""]
    ...
    lines += ["", "*Generated by `arc analyze dashboard`*", ""]
