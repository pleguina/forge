"""arc.analyze.hls_reports.formatter
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
from arc.analyze.hls_reports.extractor import HLSModuleReport
...
def to_markdown(reports: List[HLSModuleReport], out_path: Path) -> None:
    ...
    lines += [
        "",
        f"*{len(reports)} module(s) — generated by `arc analyze hls-report`*",
        "",
    ]
    out_path.write_text("\n".join(lines))
...
def to_html(reports: List[HLSModuleReport], out_path: Path) -> None:
    ...
    html = (
        ...
        + "\n".join(row_html)
        + "\n</table>"
        "<footer>Generated by <code>arc analyze hls-report</code></footer>"
        "</body></html>"
    )
