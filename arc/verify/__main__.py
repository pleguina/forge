...
def _cmd_preflight(args: argparse.Namespace) -> int:
    ...
    # ── Load the flow ──────────────────────────────────────────────────────
    from arc.verify.flow_loader import load_generic_flow
    from arc.verify.exceptions import FwVerifyError
...
def _bootstrap(plugin_id: str | None, flow_file: str) -> bool:
    ...
    if plugin_id:
        from arc.verify.plugin_registry import bootstrap_plugin as _bp
        try:
            _bp(plugin_id)
        except LookupError:
        ...
        from arc.verify.plugin_registry import bootstrap_from_flow as _bff
        try:
            _bff(Path(flow_file))
...
def _build_runtime_context(
    ...
    from arc.verify.runtime_context import RuntimeContext as FrameworkRuntimeContext
...
def _cmd_generate(args: argparse.Namespace) -> int:
    ...
    """
    from arc.verify.design_contract import load_verify_design, VerifyDesignContract
    from arc.verify.gen_sim import generate_testbench, generate_verify_flow_yml
    from arc.verify.layout import canonical_flow_dir
        ...
        if tb_port_map is None:
            ...
            if dut_rtl_src:
                ...
                if verilog_path.exists():
                    from arc.verify.rtl_introspection import (  # noqa: PLC0415
                        extract_and_write,
                        introspection_mode,
                        IntrospectionError,
                    )
...
from arc.verify.plugin_registry import declare_plugin_bootstrap as _declare
_declare(PLUGIN_ID, __name__)


def bootstrap() -> None:
    """Declare {plugin_id} with the framework.  Idempotent."""
    from arc.verify.plugin_registry import is_plugin_bootstrapped, mark_bootstrapped
...
def _cmd_doctor(args: argparse.Namespace) -> int:
    ...
    from arc.verify.rtl_introspection import _pyverilog_available, introspection_mode  # noqa: PLC0415
