"""topgen.stale_detection — Stale artifact detection for top generation (B4).
...
or IP packages without re-running ``topgen gen-top``.
        ...
        if not artifact_path.exists():
            ...
            if artifact_name == "build_manifest.json":
                report.missing.append(
                    ...
                    f"→ run topgen gen-top to generate"
                )
            continue
...
def check_ip_info_staleness(
    ...
    ``ip_info.yaml`` is generated by ``topgen ip-summary``.  It becomes
...
def format_stale_report(report: StaleTopGenReport) -> list[str]:
    ...
    for s in report.stale:
        ...
        lines.append(f"         → Re-run: topgen gen-top")
