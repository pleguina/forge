...
def cmd_hls_report(args) -> None:
    ...
    from arc.analyze.hls_reports.extractor import collect_reports
    from arc.analyze.hls_reports.formatter import to_csv, to_markdown, to_html
...
def cmd_latency_check(args) -> None:
    ...
    from arc.analyze.latency_static.graph import build_graph
    from arc.analyze.latency_static.checker import check_merge_points
    from arc.analyze.latency_static.reporter import render_markdown
    ...
    if hls_root:
        ...
        if hls_root_path.exists():
            from arc.analyze.hls_reports.extractor import collect_reports, latency_map_from_reports
            try:
...
def cmd_runtime_latency(args) -> None:
    ...
    from arc.analyze.latency_runtime.probe import load_probe_csv, measure_latency
    from arc.analyze.latency_runtime.comparator import compare
    from arc.analyze.latency_runtime.reporter import render_markdown

    probe_csv  = Path(args.probe_csv)
    output_path = Path(args.output)

    try:
        events = load_probe_csv(probe_csv)
    except FileNotFoundError as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        sys.exit(1)
...
def cmd_plot_results(args) -> None:
    ...
    from arc.analyze.result_plots.config import load_plot_config
    from arc.analyze.result_plots.engine import generate_plots
...
def cmd_dashboard(args) -> None:
    ...
    from arc.analyze.dashboards.aggregator import collect
    from arc.analyze.dashboards.renderer import render_html, render_markdown_summary
...
def register(sub) -> None:
    """Register the ``arc analyze`` sub-parser and all its commands."""
    import argparse
    ...
    p_analyze = sub.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  arc analyze hls-report --hls-build-root build_hls"
            " --output out/reports/hls_summary\n"
            "  arc analyze latency-check plugins/myplugin/designs/design.yml"
    ...
    p_rt.add_argument(
        ...
        help="CSV with columns: cycle, signal, value (written by arc verify run)",
    )
    p_rt.add_argument(
