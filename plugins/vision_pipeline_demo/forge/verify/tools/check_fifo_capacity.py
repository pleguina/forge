#!/usr/bin/env python3
"""Negative fixture: configure a burst/stall requirement that provably
exceeds FIFO capacity, and expect a strict failure showing both the
required and configured depth. This is a deliberately-failing scenario,
exercising the diagnostic path a real design would hit if its FIFO were
undersized for its own burst pattern.
...
28, a real high-water mark that never hit its own ceiling, so it's a
genuine (not artificially capped) measurement of what this exact burst
pattern needs, not an estimate.
...
# xsim run (never saturated) reached this real high-water mark for the
# pixel-result crossing's exact burst pattern (64 back-to-back 200MHz
# writes draining into a 125MHz packetizer).
