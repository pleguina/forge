...
# Curated VERIFICATION RESULT evidence that lives inside a flow's output
# directory but, unlike everything else there (verify.flow.yml,
# tb_algo_top.sv, xsim_work/, stimulus_current.svh), is not regenerated by
# this runner's own pipeline -- it comes from a separate, deliberately
# manual render_throughput_result*.py that reasons about known probe
# limitations (see that script's own docstring). A blanket rmtree of the
# flow directory silently destroys this checked-in evidence with nothing
# to put it back; found by a real from-scratch run during T8 productization
# closure (git showed the file as locally `D`eleted after a standard
# `--all` run, exactly what a real external user would hit).
_PRESERVED_FLOW_FILENAMES = ("throughput_result.json",)
...
def _clean(plan: Plan) -> None:
    for step in plan.steps:
        ...
        for flow in step.get("flows", []):
            flow_dir = _FORGE_ROOT / "verify" / flow["name"]
            preserved = {}
            for name in _PRESERVED_FLOW_FILENAMES:
                path = flow_dir / name
                if path.is_file():
            ...
            if preserved:
                ...
                for name, data in preserved.items():
                    (flow_dir / name).write_bytes(data)
