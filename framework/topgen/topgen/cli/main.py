"""Main entry point for topgen CLI."""
...
from ..ip.parser import collect_all, write_summary
from ..ip.matcher import load_ip_info, auto_match_ports, MatchReport
from ..ip.contract_loader import load_contracts_for_design, synthesize_ip_info
from ..generators.structural_vhdl import write_structural_vhdl
from ..generators.structural_verilog import write_structural_verilog
from ..generators.block_design import write_bd_tcl
from ..generators.sv_testbench_generator import generate_sv_testbench
from ..generators.design_parameters import write_design_parameters
from ..validation import validate_design, validate_registry
from ..diagnostics import ATGDiagnosticReport
from ..stale_detection import check_top_gen_staleness, check_ip_info_staleness, format_stale_report
...
def cmd_clean(args):
    ...
    try:
        ...
        if not design_path.exists():
            ...
            print("   → Check the design.yml path and re-run topgen clean.", file=sys.stderr)
            sys.exit(1)
        ...
        _print_cli_error(
            "Clean failed",
            e,
            hint="Check the design path, output path, and verify-design path, then retry topgen clean.",
        )
        sys.exit(1)
...
def generate_port_map(verilog_path: Path, output_path: Path, interface_metadata: dict | None = None) -> None:
    ...
    import re as _re
    import yaml as _yaml
    from ..utils.hdl_parser import _scan_verilog_ports
    from ..utils import port_signature as _sig
    ...
    port_map = {
        "format_version": "1",
        "generated_by": "topgen gen-top --mode verilog",
...
def generate_probe_map(port_map: dict, output_path: Path) -> None:
    ...
    probe_map = {
        "format_version": "1",
        "generated_by": port_map.get("generated_by", "topgen gen-top --mode verilog"),
...
def generate_tb_bindings(port_map: dict, output_path: "Path") -> None:
    ...
    lines.append("// ============================================================")
    lines.append(f"// tb_bindings.svh — Generated by topgen gen-top")
    lines.append(f"// Top module   : {top_module}")
    lines.append(f"// Port sig hash: {sig_hash}")
    lines.append("// DO NOT EDIT — regenerate: topgen gen-top ...")
    lines.append("// ============================================================")
    lines.append("")
...
def cmd_match_ports(args):
    ...
    try:
        ...
        if not ip_info_file.exists():
            ...
            print("   Run 'topgen ip-summary' first")
            sys.exit(1)
...
def cmd_lint(args):
    """Lint VHDL file(s)."""
    try:
        from ..utils.vhdl_linter import lint_vhdl_files
...
def cmd_lint_verilog(args):
    """Lint Verilog file(s)."""
    try:
        from ..utils.verilog_linter import lint_verilog_files
...
def cmd_verify_contract(args):
    ...
    import sys
    from ..ip.contract_verifier import (
        ContractVerifier,
        verify_all,
        find_contracts,
        VerifyResult,
...
def cmd_hls_run(args):
    """Run parallel HLS jobs via framework/hls/parallel_hls.sh."""
    import subprocess

    script = Path(args.parallel_hls_script).expanduser().resolve() if getattr(args, "parallel_hls_script", None) else _find_parallel_hls_script()
    if script is None:
        print(
            "ERROR: cannot locate parallel_hls.sh. "
            "Pass --parallel-hls-script or set TOPGEN_PARALLEL_HLS_SCRIPT.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if getattr(args, "jobs", None) is not None:
        cmd += ["-j", str(args.jobs)]
    if getattr(args, "hls_config", None):
        cmd += ["-p", str(args.hls_config)]
    if getattr(args, "hls_build_root", None):
        cmd += ["-b", str(args.hls_build_root)]
    if getattr(args, "ip_packages_dir", None):
        cmd += ["-i", str(args.ip_packages_dir)]
    if getattr(args, "design", None):
        cmd += ["-d", str(args.design)]
    if getattr(args, "flow", None):
        cmd += ["-f", args.flow]
    if getattr(args, "vitis_hls", None):
        cmd += ["-v", str(args.vitis_hls)]
...
def cmd_validate(args):
    ...
    try:
            ...
            if getattr(args, "check_stale", False):
                ...
                if stale_lines:
                    ...
                    stale_diag.warn(
                        ...
                        action="Re-run topgen gen-top to rebuild",
                        path=output_dir,
                    )
        ...
        _print_cli_error(
            "Validation failed",
            e,
            hint="Fix the reported design or contract issue, then re-run topgen validate.",
        )
        sys.exit(1)
...
def cmd_validate_registry(args):
    ...
    try:
        ...
        _print_cli_error(
            ...
            e,
            hint="Fix the modules.yml registry issue, then re-run topgen validate-registry.",
        )
        sys.exit(1)
...
def cmd_gen_top(args):
    ...
    try:
        ...
        if args.mode == "vhdl":
            ...
            if args.lint:
                try:
                    from ..utils.vhdl_linter import lint_vhdl_file
            ...
            if args.lint:
                try:
                    from ..utils.verilog_linter import lint_verilog_file
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="topgen",
        description="Algorithm Top Generator - Create FPGA top-level wrappers from design specifications",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Extract IP archives
  topgen unpack-ips ip_archives/ --ip-root ips/
  
  # Generate IP summary
  topgen ip-summary design.yaml
  
  # Show port connections
  topgen match-ports design.yaml
  
  # Generate structural VHDL top
  topgen gen-top design.yaml --mode vhdl --output algo_top.vhd
  
  # Generate structural Verilog top
  topgen gen-top design.yaml --mode verilog --output algo_top.v

    # Clean generated DUT and verify artifacts for one design
    topgen clean design.yaml --output out/algorithm/algo_top.v
  
  # Generate Block Design TCL
  topgen gen-top design.yaml --mode bd --output design.tcl
  
  # Generate VHDL with linting
  topgen gen-top design.yaml --mode vhdl --lint --fix-lint
  
  # Generate Verilog with linting
  topgen gen-top design.yaml --mode verilog --lint
  
  # Lint existing VHDL file(s) with GHDL
  topgen lint algo_top.vhd
  topgen lint algo_top.vhd --std 08 --ieee synopsys
  
  # Lint existing Verilog file(s)
  topgen lint-verilog algo_top.v
  topgen lint-verilog algo_top.v --strict --no-warnings
        """
    )
    
    parser.add_argument("--version", action="version", version="topgen 1.1.0")
    parser.add_argument(
        "--debug",
        action="store_true",
        default=False,
    ...
    # unpack-ips command
    p_unpack = subparsers.add_parser("unpack-ips", help="Extract IP archives")
    p_unpack.add_argument("src", type=Path, help="Directory containing IP archives (*.zip, *.tar.*)")
    p_unpack.add_argument("--consumer-root", type=Path, help="Consumer workspace root used to resolve default output paths")
    ...
    # ip-summary command
    p_summary = subparsers.add_parser("ip-summary", help="Generate IP metadata summary")
    p_summary.add_argument("design", type=Path, help="design.yaml file")
    p_summary.add_argument("--consumer-root", type=Path, help="Consumer workspace root used to resolve relative defaults")
    ...
    p_summary.add_argument("--src-root", type=Path, help="Source root for HDL files (default: design file's directory)")
    ...
    # match-ports command
    p_match = subparsers.add_parser("match-ports", help="Show port connection report")
    p_match.add_argument("design", type=Path, help="design.yaml file")
    p_match.add_argument("--consumer-root", type=Path, help="Consumer workspace root used to resolve relative defaults")
    ...
    p_validate = subparsers.add_parser("validate", help="Validate design.yml without generating")
    ...
    p_validate.add_argument(
        ...
        default=False,
        help=(
            "After contract validation, also verify that generated artifacts "
            "(algo_top.v, build_manifest.json, maturity_report.json, …) are "
            "not older than design.yml / modules.yml / ip_info.yaml."
        ),
    )
    p_validate.set_defaults(func=cmd_validate)

    # validate-registry command
    p_val_reg = subparsers.add_parser("validate-registry",
                                       help="Validate a modules.yml registry independently")
    ...
    # gen-top command
    p_gen = subparsers.add_parser("gen-top", help="Generate algorithm top-level")
    p_gen.add_argument("design", type=Path, help="design.yaml file")
    p_gen.add_argument("--consumer-root", type=Path, help="Consumer workspace root used to resolve relative defaults")
    p_gen.add_argument("--mode", choices=["vhdl", "verilog", "bd"], required=True,
    ...
    p_gen.add_argument("--hls-build-root", type=Path, help="HLS build directory used for manifest/source resolution (default: build_hls/)")
    p_gen.add_argument("--src-root", type=Path, help="Source root for HDL files (default: design file's directory)")
    ...
    p_gen.add_argument("--gen-testbench", action="store_true", help="Generate SystemVerilog testbench (Verilog mode only)")
    p_gen.add_argument("--xml-stimulus", type=Path, help="Path to XML file for stimulus generation (testbench generation)")
    p_gen.add_argument("--xml-stimulus-tool", type=Path, help="Path to the xml_to_sv_stimulus helper binary")
    p_gen.add_argument("--event-id", type=int, default=1, help="Event ID to extract from XML (default: 1)")
    p_gen.add_argument("--hls-metrics", type=Path, help="HLS metrics JSON file (from extract_hls_metrics.py)")
    p_gen.add_argument("--contracts-from", type=Path, metavar="MODULES_YML",
    ...
    p_gen.add_argument("--strict", action="store_true",
                       help="Topology A strict mode: fail if any module lacks a contract, any connection "
                            "uses auto-match or port_map_ranges, or topology groups have verification errors")
    p_gen.add_argument("--lint", action="store_true", help="Run VHDL linter after generation (requires vsg)")
    ...
    p_gen.add_argument(
        ...
        help=(
            ...
            "in the process environment before design loading so that src: paths "
            "containing that variable are resolved correctly."
        ),
    )
    p_gen.set_defaults(func=cmd_gen_top)

    # clean command
    p_clean = subparsers.add_parser("clean", help="Remove generated artifacts for one design")
    p_clean.add_argument("design", type=Path, help="design.yaml file")
    p_clean.add_argument("--consumer-root", type=Path, help="Consumer workspace root used to resolve relative defaults")
    p_clean.add_argument("--output", "-o", type=Path, help="Generated top file path used to anchor DUT artifact cleanup")
    p_clean.add_argument("--ip-info", type=Path, help="Generated ip_info.yaml path if it lives outside the output directory")
    p_clean.add_argument("--verify-design", type=Path, help="design.verification.yml to also clean generated verify artifacts")
    p_clean.add_argument("--no-verify", action="store_true", help="Only clean DUT artifacts; leave verify-generated files untouched")
    p_clean.add_argument("--dry-run", action="store_true", help="Print what would be removed without deleting files")
    p_clean.set_defaults(func=cmd_clean)
    
    # lint command
    p_lint = subparsers.add_parser("lint", help="Lint VHDL file(s) using GHDL")
    p_lint.add_argument("files", nargs="+", type=Path, help="VHDL file(s) to lint")
    p_lint.add_argument("--fix", action="store_true", help="Placeholder for compatibility (GHDL doesn't auto-fix)")
    p_lint.add_argument("--std", default="08", help="VHDL standard (87, 93, 02, 08, 19) - default: 08")
    p_lint.add_argument("--ieee", default="synopsys", help="IEEE library (standard, synopsys, mentor) - default: synopsys")
    p_lint.set_defaults(func=cmd_lint)
    
    # lint-verilog command
    p_lint_verilog = subparsers.add_parser("lint-verilog", help="Lint Verilog file(s) with Verilator (lint only, not a supported simulation backend)")
    p_lint_verilog.add_argument("files", nargs="+", type=Path, help="Verilog file(s) to lint")
    p_lint_verilog.add_argument("--top-module", help="Top module name (auto-detected if not provided)")
    p_lint_verilog.add_argument("--no-warnings", action="store_true", help="Suppress warnings, show only errors")
    p_lint_verilog.add_argument("--strict", action="store_true", help="Enable strict mode with additional checks")
    p_lint_verilog.set_defaults(func=cmd_lint_verilog)

    # resources command
    p_resources = subparsers.add_parser("resources", help="Print installed framework resource paths (currently none)")
    p_resources.add_argument("--key", help="Print only the value for a resource key when available")
    p_resources.add_argument("--format", choices=["text", "json"], default="text", help="Output format (default: text)")
    ...
    # hls subcommand group
    p_hls = subparsers.add_parser("hls", help="HLS orchestration commands (gen-tcl, run)")
    hls_sub = p_hls.add_subparsers(dest="hls_command", required=True, help="HLS sub-command")

    # hls gen-tcl
    p_hls_gen = hls_sub.add_parser("gen-tcl", help="Generate Vitis HLS TCL scripts from a plugin catalog")
    p_hls_gen.add_argument("module", nargs="?", default="all", help="Module name or 'all' (default: all)")
    p_hls_gen.add_argument("--hls-config", required=True, dest="hls_config", type=Path, help="Plugin HLS catalog YAML file")
    p_hls_gen.add_argument("--output-dir", dest="output_dir", type=Path, default=Path("build_hls"), help="Output directory for generated TCL (default: build_hls/)")
    p_hls_gen.add_argument("--ip-packages-dir", dest="ip_packages_dir", type=Path, help="Directory for exported IP archives")
    p_hls_gen.add_argument("--template-dir", dest="template_dir", type=Path, help="Override .tcl.j2 template directory")
    ...
    # hls run
    p_hls_run = hls_sub.add_parser("run", help="Run parallel Vitis HLS jobs via parallel_hls.sh")
    p_hls_run.add_argument("--stage", "-c", choices=["csim", "synth", "cosim", "export"], help="HLS stage to run")
    p_hls_run.add_argument("--modules", "-m", help="Space-separated module names (or 'all')")
    p_hls_run.add_argument("--jobs", "-j", type=int, help="Maximum parallel jobs")
    p_hls_run.add_argument("--hls-config", "-p", dest="hls_config", type=Path, help="Plugin HLS catalog YAML file")
    p_hls_run.add_argument("--hls-build-root", "-b", dest="hls_build_root", type=Path, help="HLS build root directory")
    p_hls_run.add_argument("--ip-packages-dir", "-i", dest="ip_packages_dir", type=Path, help="IP packages output directory")
    p_hls_run.add_argument("--design", "-d", type=Path, help="Design YAML (resolve modules from design)")
    p_hls_run.add_argument(
        "--flow", "-f", choices=["export", "syn", "impl"],
        help="IP export flow: export, syn (RTL only), impl (synth+P&R)",
    )
    p_hls_run.add_argument("--vitis-hls", "-v", dest="vitis_hls", type=Path, help="Path to vitis_hls executable")
    p_hls_run.add_argument(
        "--parallel-hls-script",
        dest="parallel_hls_script",
        type=Path,
        help="Explicit path to parallel_hls.sh (overrides repo-layout discovery)",
    )
