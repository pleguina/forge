...
def _cmd_preflight(args: argparse.Namespace) -> int:
    ...
    # ── Load the flow ──────────────────────────────────────────────────────
    from fw_verify.flow_loader import load_generic_flow
    from fw_verify.exceptions import FwVerifyError
    ...
    # ── Run preflight ──────────────────────────────────────────────────────
    from fw_verify.preflight import run_preflight
...
def _bootstrap(plugin_id: str | None, flow_file: str) -> bool:
    ...
    if plugin_id:
        from fw_verify.plugin_registry import bootstrap_plugin as _bp
        try:
            _bp(plugin_id)
        except LookupError:
        ...
        from fw_verify.plugin_registry import bootstrap_from_flow as _bff
        try:
            _bff(Path(flow_file))
...
def _build_runtime_context(
    ...
    from fw_verify.runtime_context import RuntimeContext as FrameworkRuntimeContext
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    if strict:
        from fw_verify.layout import validate_layout
    ...
    # ── 6. Get backend adapter ────────────────────────────────────────────────
    from fw_verify.backend_registry import get_adapter
    try:
...
def _cmd_generate(args: argparse.Namespace) -> int:
    ...
    """
    from fw_verify.design_contract import load_verify_design, VerifyDesignContract
    from fw_verify.gen_sim import generate_testbench, generate_verify_flow_yml
    from fw_verify.layout import canonical_flow_dir
        ...
        if tb_port_map is None:
            ...
            if dut_rtl_src:
                ...
                if verilog_path.exists():
                    from fw_verify.rtl_introspection import (  # noqa: PLC0415
                        extract_and_write,
                        introspection_mode,
                        IntrospectionError,
                    )
...
def _cmd_prepare(args: argparse.Namespace) -> int:
    ...
    from fw_verify.stimulus_contract import validate_stimulus
...
from fw_verify.plugin_registry import declare_plugin_bootstrap as _declare
_declare(PLUGIN_ID, __name__)


def bootstrap() -> None:
    """Declare {plugin_id} with the framework.  Idempotent."""
    from fw_verify.plugin_registry import is_plugin_bootstrapped, mark_bootstrapped
...
from fw_verify.stimulus_helpers import StimulusEmitter, write_run_stimulus_svh
...
def _cmd_doctor(args: argparse.Namespace) -> int:
    ...
    from fw_verify.diagnostics import DiagnosticReport, Severity
    ...
    from fw_verify.rtl_introspection import _pyverilog_available, introspection_mode  # noqa: PLC0415
    ...
    if contract is not None:
        from fw_verify.stimulus_contract import validate_stimulus
        from fw_verify.layout import resolve_generated_files
        from fw_verify.supported_matrix import validate_flow_matrix
        _CSIM_KINDS = {"hls_csim", "hls_cosim"}
        ...
        for flow_decl in contract.flows:
            ...
            if flow_decl.kind in _CSIM_KINDS:
                from fw_verify.backend_csim import find_tb_binary
    ...
    from fw_verify.supported_path_validator import validate_supported_path
    ...
    if contract is not None:
        try:
            from fw_verify.stale_artifact import check_flow_staleness
            for flow_decl in contract.flows:
...
def _cmd_release_check(args: argparse.Namespace) -> int:
    ...
    """
    from fw_verify.release_check import run_release_check
