...
## Phase 6 — Simple user-facing CLI

Release plan §6.1-§6.7 (lines 606-726). `forge inspect`/`forge build`
already existed (built in earlier phases); `forge init`/`forge test`/
`forge report` did not exist at all. Slice ordering and full rationale in
the phase's own planning doc; summarized per-slice below as each lands.

### Phase 6 (slice 0) — Shared output envelope + exit-code policy foundation

| Item | Status | Evidence |
|---|---|---|
| `CommandEnvelope` shape | **DONE** | New `forge/core/cli/envelope.py`: `CommandEnvelope` dataclass (`schema_version`, `status` ∈ `{pass,warn,fail,error}`, `diagnostics`, `artifacts`, `metrics`, `next_actions`), `to_dict`/`from_dict`, `exit_code(strict=)` (`pass`→0, `warn`→0 unless `--strict`→1, `fail`→1, `error`→2 — the *only* remaining meaning for exit code 2, closing the pre-Phase-6 inconsistency where `2` meant three different things across `main.py`/`core.py`/`topgen.py`/`release_check.py`). |
| `from_diagnostic_report` adapter | **DONE** | Reads any `.ok`/`.ok_strict`/`.errors`/`.warnings`/`.to_dict()["diagnostics"]`-shaped object (both `verify.diagnostics.DiagnosticReport` and `core.diagnostics.ATGDiagnosticReport` already match) and derives real 3-way status from actual WARNING/ERROR/CRITICAL counts — fixes the latent bug where `DiagnosticReport.to_dict()` only ever emitted `"pass"`/`"fail"`, never `"warn"`, even with real warnings present. Also computes de-duplicated, order-preserving `next_actions` from diagnostic `.action` strings, and populates `metrics["counts"]`/`metrics["label"]`. |
| `emit`/`render_human` | **DONE** | JSON mode prints the full envelope to stdout. Human mode preserves the pre-existing convention (`DiagnosticReport.print_console`'s split): NOTE diagnostics + summary/metrics/artifacts/next_actions go to stdout, WARNING/ERROR/CRITICAL diagnostics go to stderr — so migrating a command onto the envelope does not silently move error text off stderr for shell scripts that grep it. |
| Exit-code policy doc | **DONE** | New `docs/development/cli_exit_codes.md` — the pre-Phase-6 state of exit code `2`, the reconciled 0/1/2/`--strict` policy, and the envelope shape, cross-referenced from here rather than duplicated. |
| First real migration (the proof) | **DONE** | `verify/__main__.py::_doctor_emit` (backs both `forge verify doctor` and `forge verify release-check`) now builds a `CommandEnvelope` via `from_diagnostic_report` and calls `emit()`. `FWVxxxx` codes and `DiagnosticReport` itself untouched — bridged, not merged, per the release plan's own wording. `forge verify preflight` deliberately left alone (see Honest deferral list, item 2 below). |
| Import-direction check updated | **DONE** | `ci/import_direction_check.sh`'s Rule 1 ("nothing outside `forge/core/cli/` imports `forge.core.cli`") now carries one documented exception: `forge.core.cli.envelope` is itself CLI-layer shared code, meant to be importable from `forge/verify/__main__.py` (a separate CLI entry point) as well as `forge/core/cli/groups/*` — every other `forge.core.cli` import from `forge/verify/` is still a violation. |
| Tests | **DONE** | New `tests/test_cli_envelope.py` (20 tests: schema round-trip, exit-code mapping table incl. `--strict` promotion, status derivation off a synthetic double, `next_actions` de-dup, stream-splitting `emit()` behavior). Updated `tests/verify/test_verify_cli_commands.py`: the existing `report["status"] in ("pass","warn")` assertion is now genuinely exercisable — passthrough_demo's real fixture has 2 real warnings (stale `port_map.yaml`) and 0 errors, so the assertion is strengthened to `report["status"] == "warn"` outright; added `test_doctor_json_strict_promotes_warn_to_fail_exit_code` (a real `--strict` case: exit 1, status stays `"warn"`). |
| Full suite | **DONE** | `787 passed, 9 skipped` (+21 over Phase 5's `766 passed, 9 skipped`), coverage `61.96%`, **0 regressions**. |

### What's left after slice 0

Slices 6.1-6.7 (inspect/build envelope adoption + new flags, doctor
rationalization, `forge test`/`forge report`/`forge init`, envelope sweep
of remaining commands) — see the phase's own slice-ordering table.

### Phase 6 (slice 1) — `forge inspect`: contract maturity, next_actions, envelope

| Item | Status | Evidence |
|---|---|---|
| Shared maturity-summary helper | **DONE** | New `_compute_maturity_summary(cfg, match_report, report=None)` in `core/cli/groups/topgen.py`, factored out of `cmd_gen_top`'s inline `maturity` dict byte-for-byte (module/connection/wiring-method counts, `strict_pass`) — `cmd_gen_top` now calls it and adds only `port_signature_hash` on top (unchanged output, confirmed by the existing `test_topgen_cli_commands.py` suite passing unmodified). `forge inspect` calls the same helper with `report=None`; the port-accounting fields (`ports`, `strict_pass`) then stay honestly `None` rather than fabricated, since they require the generator's own open/tied-port report. |
| `build_project_ir_with_match_report` | **DONE** | New `forge/ir/build.py` function (alongside a refactor of `build_project_ir` into a shared `_build_project_ir_full` body) returning `(project, cfg, match_report)` — lets `forge inspect` get the `cfg`/`match_report` the maturity summary needs without reloading the design or re-running the matcher a second time. `build_project_ir`'s own signature/behavior is unchanged (confirmed: all 4 existing test files calling it directly still pass unmodified). |
| `next_actions` heuristic | **DONE** | Small, explicit mapping table in `inspect.py` (`_compute_next_actions`): any `error`-severity IR diagnostic → "Fix the errors above before generating"; any compat-mode module → "Add interface contracts or pass --strict at generation time". Deliberately not a new `DiagnosticReference.action` field (would touch many `forge/ir/build.py` construction sites for a phase about the CLI, not the IR — release plan's own scoping call). |
| Envelope adoption | **DONE** | `inspect.py`'s previous 4 different `--json` shapes (error dict / diff dict / explain-staleness dict / full IR dump) are now one `CommandEnvelope`. Plain `--json` now emits a *summary* (`status`, IR diagnostics reshaped, `metrics.counts`/`metrics.maturity`/`metrics.content_hash`, `next_actions`) — the full canonical IR is still obtainable, unchanged, via `--emit-ir`. `--diff`/`--explain-staleness` results moved into `metrics["diff"]`/`metrics["explain_staleness"]` (query results, not diagnostics). Guided early-exit errors (missing design/diff/explain-staleness file) keep their pre-existing stdout-message convention (never moved to stderr) — a deliberate, narrower choice than `verify doctor`'s stderr-routing convention, since inspect's guided errors never had that split and no bug motivated adding one. |
| Import-direction check | **N/A** | No new cross-subsystem import edge: `inspect.py` calling `core/cli/groups/topgen.py`'s helper is same-layer (both `core/cli/groups/*`), already permitted (precedent: `build.py` already imports `topgen.compute_gen_top_plan`). |
| Tests | **DONE** | `tests/test_inspect_cli_group.py`: updated the 4 JSON-shape assertions for the new envelope (deliberate — `test_inspect_json_output_is_well_formed` now also documents that passthrough_demo's real design genuinely has 2 warning diagnostics, so `status` is genuinely `"warn"`, not a placeholder `"pass"`); 3 new tests — maturity-summary presence/honest-absence on both real reference plugins, a synthetic compat-mode-module design (neither real plugin has one) proving the compat-mode `next_action`, and a synthetic duplicate-module-name design proving the error `next_action`. |
| Full suite | **DONE** | `790 passed, 9 skipped` (+3 over slice 0's `787 passed, 9 skipped`), coverage `62.15%`, **0 regressions**. |

### Phase 6 (slice 2) — `forge build`: `--strict`, `--dry-run`, provenance, envelope

| Item | Status | Evidence |
|---|---|---|
| `--strict` | **DONE** | New flag on `build.py`'s parser, threaded through to `_gen_top_namespace_from_build_args` for `--apply` (removed the hardcoded `("strict", False)` fallback — `args` now always carries the real value). Plan-only path: new `_strict_violations(ctx)` reports the same 6 checks `gen-top --strict` performs (compat-mode modules, auto-match/port_map_ranges wiring, topology-group/cardinality/CDC errors), reusing `ctx`'s already-computed `match_report`/issue lists — presentation, not new checking logic. Each violation becomes a `severity: "error"`/`category: "strict_mode"` diagnostic, which folds into the envelope's existing severity→status derivation (no separate exit-code branch needed). Confirmed via a synthetic compat-mode-module design: plain plan mode reports `status: "warn"` (exit 0), `--strict` on the identical design reports `status: "fail"` (exit 1) — proving this is genuinely new coverage, not a restatement of an already-unconditional plan error. (A separate, pre-existing behavior confirmed *unchanged* by this slice: an undeclared CDC violation was already a hard plan failure regardless of `--strict`, since `plan.unresolved_issues` errors have always failed the plan unconditionally — only `gen-top --apply`'s own generation-time check was strict-gated.) |
| `--dry-run` | **DONE** | True alias for `--apply`'s negation (`dest="apply", action="store_false", default=argparse.SUPPRESS` so it doesn't clobber `--apply`'s own default when absent) — `gen-top --dry-run` users and `forge build` users now see consistent vocabulary. Confirmed identical `plan_hash`/exit code/zero-writes vs. the pre-existing default, and that `--apply --dry-run` on the same line correctly resolves to "don't apply" (last flag for a shared dest wins). |
| `--provenance`/`--explain-staleness` | **DONE** | Reuses `forge.ir.provenance.build_provenance`/`write_provenance`/`explain_staleness` directly against `ctx.project` — the same call `inspect.py` already makes. `plan_hash` (already computed) is now `build_provenance`'s real `plan_hash=` argument, so `forge build --provenance` records a real, correct plan hash (consistent with what slice 5.1 already wired into `gen-top` itself). New `_build_command_options(args)` mirrors `topgen._gen_top_command_options`'s shape, scoped to `forge build`'s own flags. |
| Envelope adoption | **DONE** | `{"plan": asdict(plan), "plan_hash": h}` (a full plan dump) becomes one `CommandEnvelope`: `metrics.counts` carries connection/transformation/compat-mode-module counts (deliberately *not* the full connection lists — same "summary over full-dump" tradeoff slice 6.1 made for `forge inspect`), `artifacts = plan.output_artifacts` (+ the provenance path when written), `diagnostics = plan.unresolved_issues` reshaped (already had `severity`/`category`/`message`, close to the target shape already) plus any `--strict` violations. The bespoke `--accept-plan-hash` mismatch dict (`{"error": ..., "expected": ..., "actual": ...}`) is folded into the same envelope too (`metrics.expected_plan_hash`/`metrics.actual_plan_hash`), for one consistent shape across every `forge build` exit path. |
| Tests | **DONE** | `tests/test_build_cli_group.py`: updated the 4 existing JSON-shape assertions for the new envelope (deliberate); 7 new tests — CDC-design proof that pre-existing plan-error behavior is unchanged by `--strict`, compat-mode-module proof that `--strict` adds genuinely new coverage, `--dry-run` alias equivalence (incl. `--apply --dry-run` combined), and a real `--provenance`/`--explain-staleness` round-trip on passthrough_demo. |
| Full suite | **DONE** | `797 passed, 9 skipped` (+7 over slice 1's `790 passed, 9 skipped`), coverage `62.27%`, **0 regressions**. |

### Phase 6 (slice 3) — Doctor/release-check rationalization (§6.6)

| Item | Status | Evidence |
|---|---|---|
| Fix the "warn" bug, for real, everywhere | **DONE** | Slice 6.0 already fixed `_doctor_emit` (backs `forge verify doctor`/`release-check`); this slice finishes the doctor family by moving `forge doctor` (`core/cli/groups/doctor.py`) onto the same `CommandEnvelope`/`from_diagnostic_report` bridge — its bespoke `{"checks","ok"}` dict is gone. `forge verify preflight` deliberately untouched (see below). |
| De-duplicate tool-availability checks | **DONE** | New `forge.core.toolchain_versions.tool_present(name)` (thin `shutil.which` wrapper, reusing the module's existing tool list from Phase 5 slice 5.1). All 3 independent `xvlog`/`xelab`/`xsim` presence checks found in this investigation — `core/cli/groups/doctor.py`, `verify/__main__.py`'s `_cmd_doctor`, and `verify/release_check.py`'s `check_tools` branch (a 3rd instance found while implementing, beyond the 2 the plan's own investigation had named) — now all call it. Confirmed via a new cross-CLI test that `forge doctor` and `forge verify doctor` report the identical tool-availability verdict for this environment. |
| `--strict` becomes real for `forge doctor` | **DONE (behavior change, documented)** | Pre-envelope, `forge doctor --strict` was dead code — it only escalated `"missing (required)"` checks, and no check has ever set `required=True` (every check is an optional extra, per the module's own docstring), so `--strict` never actually changed the exit code. Adopting the shared envelope's `--strict` semantics (any WARNING promotes to a failure) makes `--strict` genuinely useful for the first time — confirmed failing in this sandbox (matplotlib/numpy/networkx absent) where it previously always passed. A real, deliberate behavior change, not a bug fix regression: no test previously exercised `--strict`'s outcome (only that plain `forge doctor` never fails), so nothing broke. |
| Compatibility aliases / containment | **DONE** | No commands removed or merged. Documented containment: **`forge doctor`** — plugin-agnostic installation/toolchain health (Python version, xvlog/xelab/xsim/ghdl/verilator on PATH, optional Python extras, framework resource files); authoritative for "is my local forge install sane". **`forge verify doctor`** — one plugin's flow artifacts against a specific `design.verification.yml` (contract loading, bootstrap/gen_stimulus presence, RTL introspection mode, per-flow artifact completeness, stimulus contract validity, canonical layout, stale-artifact detection, supported-path gate); authoritative for "is this plugin's verify setup ready to run". **`forge verify release-check`** — runs all RC-01…RC-10 release-readiness criteria in one call (a superset gate for release sign-off, not a development-loop tool); authoritative for "is this plugin release-ready". **`forge verify preflight`** — a real, narrower subset of `forge verify doctor`'s checks (flow-loading + a quick pre-run sanity pass), kept as a separate, unmerged command per rule 5/§6.6's explicit "do not remove useful existing commands without compatibility aliases" — merging it into `forge verify doctor` would be a bigger rewrite than this phase's scope justifies, and it serves a genuinely different, faster use case (a quick check before `forge verify run`, not a full health audit). |
| Tests | **DONE** | `tests/test_doctor_cli_group.py`: updated the 2 JSON-shape assertions for the new envelope (deliberate); new `--strict` failure test; new cross-CLI test proving `forge doctor` and `forge verify doctor` agree on tool availability. `tests/test_toolchain_versions.py`: 2 new `tool_present` tests (fake-PATH, environment-independent, matching this module's existing testing convention). |
| Full suite | **DONE** | `801 passed, 9 skipped` (+4 over slice 2's `797 passed, 9 skipped`), coverage `62.31%`, **0 regressions**. |

### Phase 6 (slice 4) — `forge test` (new)

| Item | Status | Evidence |
|---|---|---|
| Signature-widened event-selection resolver | **DONE** | `verify/__main__.py`'s `_resolve_xml_run_selection` (argparse-only) split into a new `_resolve_xml_run_selection_from_values(cfg, *, event_id=, event_list=, ...)` (plain kwargs, no `argparse.Namespace` needed) plus a thin argparse-based wrapper kept for `forge verify run`'s existing call site — confirmed behavior-preserving (all 33 `tests/verify/` tests, including the real-xsim ones, pass unmodified). |
| New `forge/core/cli/groups/test.py` | **DONE** | Three modes. `check-only`/`prepare` both delegate to the existing `_cmd_prepare` (generate + validate-stimulus + validate-layout, no execution) — `check-only` is a fixed-mode call (no `--dry-run`/`--strict`/`--port-map`), `prepare` exposes `_cmd_prepare`'s full knobs; genuinely the same underlying call under two names for two different mental models (a quick gate vs. the full configurable delegate), not a fake distinction. `run` is the new work (below). |
| Per-event results | **DONE** | New `_resolve_event_ids(selection)`: `--event-list`/`--event-id` take precedence (parses `"1,7,10-12"`-style ranges); otherwise every `<event id="...">` in the dataset XML is enumerated (tag-name-agnostic root, confirmed working against both real reference plugins' differently-named root elements) and run **individually** — one simulation per event, unlike `forge verify run --all-events` (which feeds every event through *one* simulation; confirmed by reading `verify/__main__.py`'s own `--all-events` help text). Each event builds its own single-event `XmlRunSelection` + `_build_runtime_context` + calls the *unchanged* `_run_one_loaded_flow` — zero changes to that function's signature or the backend/checker contract. `--all-dataset-parts` falls back to the existing `_run_dataset_parts` aggregate path unchanged (honest deferral — see below). |
| JUnit XML | **DONE** | New `forge/verify/junit_xml.py::write_junit_xml` — stdlib `xml.etree.ElementTree` only, no new dependency. Exposed as `forge test run --junit-xml <path>`. (`forge verify run --junit-xml` symmetry mentioned in the phase's own planning doc was not added — `forge verify run` executes one flow's whole selection as a single unit, with no per-event loop to report, so a JUnit file there would only ever have one `<testcase>`; deferred as not worth the surface area until a real need appears.) |
| JSON-mode output hygiene | **DONE (found during implementation)** | `_cmd_prepare`/`_run_one_loaded_flow`/`_run_dataset_parts` all print narrative progress unconditionally (no `emit_progress`-style flag, unlike `compute_gen_top_plan`) — a real bug this slice's own tests caught: `--json` output had narrative text before the JSON blob, breaking JSON parsing. Fixed with a small `_maybe_quiet_stdout(json_mode)` context manager that redirects stdout to a throwaway buffer around each delegated call only when `json_mode` — stderr untouched (never carries structured data). |
| Envelope adoption | **DONE** | `metrics`: `{"events_run", "events_passed", "events_failed"}` for `run`; `{"mode": "check-only"|"prepare"|"all_dataset_parts"}` for the other modes. `artifacts`: the JUnit XML path + each event's xvlog/xelab/simulate log paths (from the adapter's own `describe_backend_outputs`). `diagnostics`: one `FWV013` entry per failed event. |
| Tests | **DONE** | New `tests/test_test_cli_group.py` (8 tests, real end-to-end against passthrough_demo's real 2-event golden XML using the real XSIM toolchain confirmed present in this environment — not CSim-stubbed): both `check-only`/`prepare` real generate+validate+layout passes; a single real event; **2 distinct real per-event xsim runs** via `--all-events` with a valid 2-`<testcase>` JUnit XML; `--event-list`; a real preflight-failure-as-per-event-failure regression guard (mirroring `test_verify_run_xsim.py`'s analogous test); CLI registration/argument-validation checks. Also fixed `tests/test_cli_help_consistency.py`'s group-listing assertion (now includes `test`). |
| Full suite | **DONE** | `813 passed, 9 skipped`, coverage `62.59%`, **0 regressions**. |

**Honest deferral confirmed by this slice**: `--all-dataset-parts` flows (part-file-level datasets, not event-level) keep today's aggregate-only reporting — no per-event breakdown, no JUnit XML for that mode (nothing per-event to report). A true per-event breakdown for those would need a larger dataset-format change out of this phase's scope (see the phase's own honest-deferral list, item 4).

**Addendum (found during slice 6.6's real end-to-end `forge init` testing, fixed here)**: the per-event loop above did not regenerate `stimulus_current.svh` between events. `forge verify prepare`'s own stimulus-contract check only *checks* that this file exists — generating it is a separate, plugin-owned step (`tools/gen_stimulus.py`), never invoked automatically by the framework's run path. Without a fix, a `forge test run --all-events`/`--event-list` invocation would silently re-simulate whichever event's stimulus happened to already be on disk for every event, reporting false per-event passes. Fixed with a new `_regenerate_stimulus_for_event(flow_name, event_id, flow_dir)` that imports the plugin's own `gen_stimulus` module (already on `sys.path` via `_bootstrap`) and calls `generate_for_flow()` before each event's simulation — best-effort (a csim-only flow with no `gen_stimulus.py` on `sys.path` just proceeds with whatever's already there, same as `forge verify run` always has). New regression test in `tests/test_test_cli_group.py` asserts the final `stimulus_current.svh` reflects the *last* event actually run, not stale data from the first.

### Phase 6 (slice 5) — `forge report` (new)

| Item | Status | Evidence |
|---|---|---|
| New `forge/core/cli/groups/report.py` | **DONE** | A thin orchestrator, not new rendering logic — every section calls a renderer that already exists: maturity (slice 6.1's `_compute_maturity_summary`, shared not duplicated, plus a new `render_maturity_markdown` next to it), latency check (`forge.analyze.latency_static`), HLS summary (`forge.analyze.hls_reports`, honest absence confirmed real on both reference plugins — neither has HLS build artifacts today), runtime latency (`forge.analyze.latency_runtime`), and the dashboard (`forge.analyze.dashboards.aggregator.collect`/`renderer.render_html`/`render_markdown_summary`, run over everything this command just wrote). |
| Provenance summary | **DONE** | New `forge.ir.provenance.render_markdown(manifest)` — pure presentation over an already-built `ProvenanceManifest` (toolchain versions, source/output hashes, command options), no new hashing/comparison logic. `forge report --provenance <path>` reads an existing manifest (from `forge build`/`forge inspect --provenance`) and renders it; honest "omitted" note when no manifest is given. |
| Verification results (reused, not recomputed) | **DONE** | New `forge.verify.junit_xml.render_markdown(path)` — parses a JUnit XML file `forge test run --junit-xml` already wrote (release-plan Phase 6 §6.4) into a Markdown pass/fail table. Since `forge report` runs as a separate process from `forge test run`, the JUnit XML file is the only honest way to "reuse, not recompute" across that process boundary — confirmed end-to-end (gen-top → `forge test run --junit-xml` → `forge inspect --provenance` → `forge report --junit-xml ... --provenance ...`, all real, on passthrough_demo with real xsim). Honest "no verification results yet — run `forge test run`" note (both in the file itself and in `next_actions`) when no JUnit XML is given. |
| Explicit, documented deferral | **DONE** | Topology SVG and the interactive design explorer are not implemented — `next_actions` always carries a clear note (confirmed present, not silently missing, by a dedicated test assertion) rather than the report silently omitting them. |
| Envelope adoption | **DONE** | `artifacts` lists every file actually written (varies by which optional sections had real input); `metrics` aggregates maturity/latency-mismatch/HLS-module/runtime-comparison counts; `diagnostics` carries each honest-absence note (`note` severity) and any sub-step warning (e.g. a real latency mismatch). |
| Tests | **DONE** | New `tests/test_report_cli_group.py` (5 tests): real end-to-end bundles on both reference plugins (asserting every claimed artifact file exists and is non-empty, and that the topology-SVG/explorer deferral note is genuinely present); a real xsim-gated end-to-end test proving the `--provenance`/`--junit-xml` reuse paths against real artifacts from `forge inspect`/`forge test run`; guided-error and CLI-registration checks. |
| Full suite | **DONE** | `819 passed, 9 skipped` (+6 over slice 4's `813 passed, 9 skipped`), coverage `63.03%`, **0 regressions**. |

### Phase 6 (slice 6) — `forge init` (new)

| Item | Status | Evidence |
|---|---|---|
| Verify-side scaffolder templates fixed | **DONE** | `verify/__main__.py`'s `_DESIGN_VERIFICATION_TEMPLATE`/`_BOOTSTRAP_TEMPLATE`/`_GEN_STIMULUS_TEMPLATE` replaced with plugin-id-parametrized versions of `plugins/passthrough_demo/forge/verify/`'s real, working files (`design.verification.yml`, `tools/bootstrap.py`, `tools/gen_stimulus.py`) — directly justified by the confirmed byte-for-byte match between the topgen-side RTL stub and passthrough_demo's real RTL. New `_GOLDEN_XML_TEMPLATE` replaces the old inline stub XML (`<!-- TODO: add event data here -->`) with passthrough_demo's real 2-event golden data, parametrized. The old templates declared an `hls_csim`/`single_module_rtl` flow with `dut_rtl_source: build_hls/...` and literal `# TODO` stimulus placeholders — structurally incompatible with the topgen-side scaffolder's plain-RTL, non-HLS module; the fixed templates declare a `full_chip_rtl`/`xsim` flow against `dut_rtl_source: gen-top/design_{plugin_id}`, matching what the topgen-side scaffold + `forge build --apply` actually produce. |
| New `forge/core/cli/groups/init.py` | **DONE** | `forge init <plugin_id>` chains 7 real steps — each one a genuine `forge <...>` CLI invocation (argv constructed and run in-process through the same parser a real user's shell would use, not a hand-built call into internals): topgen scaffold → verify scaffold → `forge topgen validate` → `forge build --apply` → `forge test prepare` → `forge test run --junit-xml` → `forge report --junit-xml`. Any step failing halts immediately with a diagnostic naming exactly which step failed (rule 11) — confirmed via `steps_completed` in the envelope's `metrics`, which only ever lists steps that actually succeeded. `--dry-run` propagates through the two scaffolders (their own existing dry-run support) and skips validate/build/test/report entirely, since there's nothing yet to validate/build/test/report on. |
| Real correctness bug found and fixed (in slice 6.4's `forge test run`, surfaced by this slice's real end-to-end testing) | **DONE** | `forge test run`'s per-event loop never regenerated `stimulus_current.svh` between events — `forge verify prepare`'s own stimulus check only *verifies* the file exists, generation is a separate plugin-owned step (`tools/gen_stimulus.py`) the framework never calls automatically. Without a fix, running event 1 after event 0 would silently re-simulate event 0's stale stimulus and still report a false pass. Fixed with a new `_regenerate_stimulus_for_event()` in `core/cli/groups/test.py` that imports the plugin's own `gen_stimulus` module (already on `sys.path` via `_bootstrap`) and calls `generate_for_flow()` before each event — confirmed via a new regression test asserting the final `stimulus_current.svh` reflects the last event actually run, and via this slice's own real `forge init` test (both scaffolded events genuinely pass with distinct golden data). |
| Real test-isolation bug found and fixed (pre-existing, surfaced by adding this slice's new test file alongside slices 6.4/6.5's) | **DONE** | Plugin bootstrap imports each plugin's `tools/bootstrap.py`/`gen_stimulus.py`/`flow_config.py` via bare, non-namespaced module names after inserting that plugin's `tools/` directory onto `sys.path`. Python caches modules by name, so once 4 different test files each began copying/scaffolding a plugin into their own `tmp_path` (`test_verify_run_xsim.py`, `test_test_cli_group.py`, `test_report_cli_group.py`, this slice's `test_init_cli_group.py`), a later test could silently receive an *earlier* test's cached module — e.g. running `throwaway_plugin`'s `gen_stimulus.py` while believing it was passthrough_demo's, generating wrong DUT signal names. New `tests/conftest.py`: an autouse fixture clearing `sys.modules` entries for those 4 module names plus `plugin_registry`'s bootstrap-declaration state around every test, re-registering the framework-owned xsim/csim backend defaults afterward (`_reset_for_testing()` clears those too, and they're only ever auto-registered once, at `forge.verify`'s first import). |
| Tests | **DONE** | New `tests/test_init_cli_group.py` (3 tests): the literal acceptance bar — `forge init throwaway_plugin`, zero manual edits, a real passing JUnit XML (`tests="1"`, `failures="0"`) and a real non-empty report bundle both exist at the end, plus real `build --apply` artifacts; a `--dry-run` test asserting the two scaffolders' own dry-run preview runs but nothing is written and validate/build/test/report never even start. |
| Full suite | **DONE** | `823 passed, 9 skipped` (+4 over slice 5's `819 passed, 9 skipped`, after the conftest.py isolation fix — see above), coverage `63.89%`, **0 regressions**. |

### Phase 6 (slice 7) — Envelope sweep + exit-code policy doc

| Item | Status | Evidence |
|---|---|---|
| `forge topgen validate` / `validate-registry` | **DONE** | New shared `_validators_to_envelope`/`_validation_error_to_diagnostic` helpers reshape `ValidationError` (topgen/validation.py) into the envelope's diagnostic shape (`code=None` — honest absence, no stable code table exists for these). `status` is the honest 3-way pass/warn/fail; `--strict`'s promotion is applied only via `CommandEnvelope.exit_code(strict=)`/`emit(..., strict=)`, never baked into the computed status (matching slice 6.0's "status string never changes under `--strict`, only the exit code does" contract — an early draft of this slice got that backwards and was corrected before landing). `--check-stale`'s `metrics.stale` keeps its pre-existing `{"count","artifacts"}` shape unchanged. |
| `forge core resources` / `verify-contract` | **DONE** | `resources`: bespoke `{key: {path,exists}}` moved to `metrics.resources`; the pre-existing `--format json` flag kept as a deprecated alias for the new `--json`. `verify-contract`: new `--json`; real contract-verification errors reshaped into `diagnostics`. **Exit-code bug fixed**: `VerifyResult.exit_code()`'s own errors→`2` convention (pre-existing, inherited by the CLI layer) conflated a real validation failure with a usage error — now `status: "fail"` → exit `1`, matching the reconciled policy; `2` is reserved for genuine usage errors (missing files, no `--contract`/`--all-contracts` given) and unknown `--key` values (also reconciled: was exit `1`, now `2`, since a bad key is a usage error). |
| `forge analyze *` (5 subcommands) | **DONE (lighter touch, as scoped)** | New `--json` on `hls-report`/`latency-check`/`runtime-latency`/`plot-results`/`dashboard`, via a shared `_emit_analyze_envelope` helper that only activates under `--json` (non-JSON text output and exit codes deliberately unchanged — these are report-generation commands, not pass/fail gates, matching the plan's own "metrics/artifacts matter more than diagnostics/status here" scoping). `metrics`/`artifacts` carry each command's real counts/output paths; `latency-check`'s real mismatches become `diagnostics` entries. |
| `docs/development/cli_exit_codes.md` finished | **DONE** | Full migration-status table (every command that adopted `CommandEnvelope` across all of Phase 6, which slice, and what changed) plus the two reconciled-exit-code bugs found and fixed in this slice, and an explicit list of what's deliberately *not* migrated (`forge verify preflight`, detailed subsystem commands like `gen-top`/`hls *`/`framework *`, `forge topgen migrate`) and why. Cross-referenced from `forge --help`'s own epilog (top-level, not repeated in every subcommand's `--help` — the doc is the single source of truth, matching this document's own header). |
| Cross-command consistency test | **DONE** | New `tests/test_cli_envelope_consistency.py` — 9 real commands (`doctor`, `inspect`, `build`, `topgen validate`/`validate-registry`, `core resources`/`verify-contract`, `analyze latency-check`, `verify doctor`) each invoked for real and asserted to produce a `--json` payload that round-trips losslessly through `CommandEnvelope.from_dict`/`to_dict` and shares the same `schema_version` — proof the sweep actually landed everywhere claimed, not just in each command's own dedicated shape-migration tests. |
| Tests | **DONE** | Updated shape-assertion tests in `test_topgen_cli_commands.py` (validate/validate-registry, incl. the two `--check-stale` tests), `test_core_cli_group.py` (resources/verify-contract, incl. the 2 reconciled exit codes), `test_analyze_cli_group.py` (+3 new `--json` tests: hls-report warn-status, latency-check pass-status, dashboard). |
| Full suite | **DONE** | `835 passed, 9 skipped` (+12 over slice 6's `823 passed, 9 skipped`), coverage `63.94%`, **0 regressions**. |

## Phase 6 closing summary

All 8 slices (6.0–6.7) complete. Starting baseline (Phase 5's own exit
state): `766 passed, 9 skipped`, coverage `61.88%`. Ending state:
`835 passed, 9 skipped`, coverage `63.94%` — **+69 tests, +2.06% coverage,
0 regressions across the whole phase**, each slice's own full-suite run
confirmed clean before moving to the next.

**What shipped**: the shared `CommandEnvelope`/exit-code policy
(`forge/core/cli/envelope.py`, `docs/development/cli_exit_codes.md`);
`forge inspect`/`forge build` migrated onto it plus new flags (`--strict`,
`--dry-run`, `--provenance`/`--explain-staleness` on `build`; contract
maturity/`next_actions` on `inspect`); the doctor family rationalized
(shared `tool_present()`, the "warn never produced" bug fixed everywhere,
containment documented); three genuinely new commands — `forge test`
(check-only/prepare/run, real per-event results, JUnit XML), `forge
report` (maturity/latency/verification/provenance sections + dashboard,
one honest deferral note for topology SVG/explorer), and `forge init`
(unifies both plugin scaffolders, fixes the verify-side templates to be
immediately runnable, chains the whole golden path with zero manual
edits) — plus the envelope swept across the remaining `--json`-supporting
commands (`topgen validate`/`validate-registry`, `core resources`/
`verify-contract`, all 5 `analyze` subcommands).
...
the "warn" status never being produced by `DiagnosticReport.to_dict()`
(slice 6.0); a 3-way tool-availability-check duplication across
`doctor.py`/`verify/__main__.py`/`release_check.py` (slice 6.3); `forge
test run`'s per-event loop silently re-simulating stale stimulus instead
of regenerating it per event (slice 6.6); a cross-test-file module-cache
contamination bug in plugin bootstrap that only surfaced once enough
tmp_path-scoped test files existed side by side (slice 6.6); two
exit-code-`2` conflations in `core verify-contract`/`core resources --key`
(slice 6.7).

**Honest deferrals** (documented, not fabricated — see each slice's own
closure entry above for the reasoning): topology SVG and the interactive
design explorer (Phase 8's job — `forge report` says so explicitly);
`forge verify preflight` stays a separate command, not merged into the
doctor family; `ATGxxxx`/`FWVxxxx` diagnostic code families stay separate
registries, bridged onto one envelope shape rather than merged;
per-event results require an event-addressable dataset (`--all-dataset-parts`
flows keep today's aggregate-only reporting); a freshly-`forge init`'d
