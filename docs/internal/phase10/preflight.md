...
| 10.7B | Generic platform wrapper (`forge framework import/io-resolve/emit-payload` applied to this plugin, without detector-specific terminology, spec §26) |
| 10.7C | Provenance/plan-hash CLI-workflow tests for this plugin; visual explorer (`forge inspect --explorer/--dot`) applied to this plugin for the first time |
| 10.7D | Documentation (tutorial pages beyond 10.1 scope), clean-user workflow (README/`run_vision_pipeline_demo.sh` covering all real flows), full CI wiring |
...
- Mailbox-transfer pipelining (more than one outstanding transaction,
  ...
  implementation.
- 10.7B (generic platform wrapper), 10.7C (provenance/plan-hash CLI-workflow
  tests and visual explorer application for this plugin), and 10.7D
  (documentation/README/CI wiring beyond 10.1 scope) — all still open,
  deferred from the 10.7 sub-slice split (§11) to later sessions.
