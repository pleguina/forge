"""Mechanical MkDocs pipeline tests.
...
that ``exclude_docs:`` is supposed to keep out of the public site is truly
absent from the built ``site/`` output — not just missing from ``nav:``,
which would not actually stop MkDocs from rendering it.
"""
...
def test_release_readiness_audit_trail_is_excluded(tmp_path: Path) -> None:
    """Regression guard: no raw session-log/audit-trail content (however
    it's named) should ever leak into the public site."""
    site_dir = _build_site(tmp_path)
...
def test_internal_directory_is_excluded(tmp_path: Path) -> None:
    """``docs/internal/`` is reserved for tracked-in-git material that
    isn't public-site content — same exclusion contract as
    ``docs/plan/``, enforced even when the directory is currently empty."""
    site_dir = _build_site(tmp_path)
