"""
Migration tooling (release-plan §2.8).
...
# anywhere else in the codebase — the two subsystems are kept independent,
# per Phase 0.6's dependency-boundary intent). This value is kept manually
...
def unified_diff_text(before: str, after: str, path: Path) -> str:
    """A readable unified diff between *before* and *after* (release-plan
    §2.8's "readable diff" requirement), for any text-content migration."""
...
def infer_contract_skeleton(module_name: str, ip_info_entry: Dict[str, Any]) -> str:
    """Generate a conservative *.interface.yaml skeleton for a
    compat-mode module (one with no contract, per
    ``MatchReport.compat_mode_modules``).

    Only clock_primary/reset_primary are inferred (from the same
    conservative name heuristics auto_match_ports already uses) —
    everything else is listed as an explicit TODO. Data-port role
    semantics can never be safely guessed from port names alone.
    """
    ...
    lines = [
        ...
        "# Generated by `forge topgen migrate --kind infer-contract` from observed",
        "# ip_info ports — release-plan §2.8 \"compatibility-mode contract inference\".",
        "#",
        "# Only clock/reset roles are inferred automatically. Data-port roles below",
        "# are listed for reference ONLY and MUST be assigned by hand: their",
        "# semantics (wiring_kind, direction, coordinates, protocol) cannot be",
        "# safely guessed from port names alone. See docs/IP_INTERFACE_POLICY.md.",
        "",
        "ip_interface:",
    ...
    remaining = sorted(port_names - consumed)
    if remaining:
        lines.append("    # TODO: assign roles for the remaining ports observed in ip_info:")
        for name in remaining:
            p = port_by_name[name]
            lines.append(f"    #   {name} (direction={p.get('direction')}, width={p.get('width')})")
