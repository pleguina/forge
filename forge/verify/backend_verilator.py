...
to ``backend: verilator`` (see ``forge.verify.supported_matrix``).
...
This module owns the generic Verilator simulation lifecycle:
  ...
  * Source-file-list generation from flow config (reusing
    :mod:`forge.verify.manifest_compile` for the manifest-driven case, the
...
So this backend only ever reports :class:`~forge.verify.execution_stage.ExecutionStage`
...
def _paths_from_prj_entries(entries: list[str]) -> list[Path]:
    ...
    Reuses :func:`forge.verify.manifest_compile.prepare_from_manifest` (HLS
...
class VerilatorBackend(BackendAdapter):
    ...
    def prepare_backend_inputs(self, cfg: Any, ctx: Any) -> None:
        ...
        If :attr:`~forge.verify.flow_loader.FlowConfig.dut_manifest` is set,
        delegates the manifest walk itself to
        :func:`forge.verify.manifest_compile.prepare_from_manifest` (same
...
def _run_logged(cmd: list[str], log_file: Path, cwd: Path | None = None) -> LoggedRun:
    ...
    Delegates to :mod:`forge.verify.subprocess_wrapper`, matching the xsim
