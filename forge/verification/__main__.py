...
def _cmd_preflight(args: argparse.Namespace) -> int:
    ...
    from forge.verification.exceptions import ForgeVerifyError
    ...
    # ── Run preflight ──────────────────────────────────────────────────────
    from forge.verification.preflight import run_preflight
...
def _bootstrap(plugin_id: str | None, flow_file: str) -> bool:
    ...
    if plugin_id:
        from forge.verification.plugin_registry import bootstrap_plugin as _bp
        try:
            _bp(plugin_id)
        except LookupError:
        ...
        from forge.verification.plugin_registry import bootstrap_from_flow as _bff
        try:
            _bff(Path(flow_file))
...
def _build_runtime_context(
    ...
    from forge.verification.runtime_context import RuntimeContext as FrameworkRuntimeContext
...
def _run_one_loaded_flow(
    ...
    if strict:
        from forge.verification.layout import validate_layout
...
def _cmd_generate(args: argparse.Namespace) -> int:
    ...
    """
    from forge.verification.design_contract import load_verify_design, VerifyDesignContract
    from forge.verification.gen_sim import generate_testbench, generate_verify_flow_yml
        ...
        if tb_port_map is None:
            ...
            if dut_rtl_src:
                ...
                if verilog_path.exists():
                    from forge.verification.rtl_introspection import (  # noqa: PLC0415
                        extract_and_write,
                        introspection_mode,
                        IntrospectionError,
                    )
...
def _cmd_doctor(args: argparse.Namespace) -> int:
    ...
    from forge.verification.diagnostics import DiagnosticReport, Severity
    ...
    from forge.verification.rtl_introspection import _pyverilog_available, introspection_mode  # noqa: PLC0415
    ...
    if contract is not None:
        from forge.verification.stimulus_contract import validate_stimulus
        from forge.verification.layout import resolve_generated_files
        from forge.verification.supported_matrix import validate_flow_matrix
        _CSIM_KINDS = {"hls_csim", "hls_cosim"}
        ...
        for flow_decl in contract.flows:
            ...
            if flow_decl.kind in _CSIM_KINDS:
                from forge.verification.backend_csim import find_tb_binary
            ...
            if gen_files.port_map.exists():
                try:
                    ...
                    from forge.verification.gen_sim import _flatten_ports  # noqa: PLC0415
    ...
    if contract is not None:
        try:
            from forge.verification.stale_artifact import check_flow_staleness
            for flow_decl in contract.flows:
...
def _cmd_release_check(args: argparse.Namespace) -> int:
    ...
    """
    from forge.verification.release_check import run_release_check
