...
.. deprecated::
    Use :mod:`forge.verification.rtl_introspection` directly.  This module now
...
Public API (shim — delegates to forge.verification.rtl_introspection)
-----------------------------------------------------------
...
from forge.verification.rtl_introspection import (
...
def parse_hls_verilog_ports(verilog_path: Path) -> list[dict[str, Any]]:
    ...
    Delegates to :func:`forge.verification.rtl_introspection.extract_ports` using
    ``mode="regex"`` (HLS-output fallback) for backward compatibility.
    Prefer :func:`forge.verification.rtl_introspection.extract_ports` with
...
def write_port_map_yaml(ports: list[dict[str, Any]], out_path: Path) -> None:
    ...
    Delegates to :func:`forge.verification.rtl_introspection.write_port_map_yaml`.
