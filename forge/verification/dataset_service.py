...
:mod:`forge.verification.dataset_format` (layer A: format loaders, envelope,
hashing) and :mod:`forge.verification.dataset_adapter` (layer B: project-owned
...
Before this module, two real bypasses existed:
  ...
  * A plugin's own ``gen_stimulus.py`` (the ``svh_include`` stimulus
    path) called layer A's :func:`~forge.verification.dataset_format.get_format_loader`
...
from forge.verification.dataset_adapter import CanonicalDataset, DatasetSource, get_dataset_adapter
from forge.verification.dataset_format import SerializedDataset, compute_events_content_hash
...
class DatasetService:
    ...
    def load(self, source: DatasetSource) -> SerializedDataset:
        ...
        :class:`~forge.verification.dataset_adapter.DatasetSource`'s own
    ...
    def compute_identity(self, dataset: CanonicalDataset) -> str:
        ...
        :func:`~forge.verification.dataset_format.compute_events_content_hash`
