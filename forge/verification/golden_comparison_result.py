...
:class:`~forge.verification.golden_model.GoldenModelProvider`'s output, plus the
...
returned :class:`~forge.verification.golden_model.ExpectedDataset`, but that
...
(in ``forge.verification.golden_model``) closes that gap by writing a small
JSON sidecar next to the generated stimulus; :func:`build_golden_comparison_result`
here joins that sidecar with a real :class:`~forge.verification.results.FlowResult`
...
from forge.verification.results import CheckResult, FlowResult
...
def build_golden_comparison_result(
    ...
    provider-identity sidecar :func:`~forge.verification.golden_model.write_provider_provenance`
...
def render_golden_comparison_markdown(payload: "dict[str, Any]") -> str:
    ...
    ``forge.verification.results.render_results_markdown``."""
