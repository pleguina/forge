...
def unpack_ip_archives(
    ...
    for forge in archives:
        mod_name = forge.stem.split(".")[0]
        ...
        if dst_mod.exists():
            if force:
                ...
                if verbose:
                    print(f"↷  {forge.name:30}  (skip – dest exists; use force=True)")
                continue
        
        dst_mod.mkdir(parents=True)
        
        if verbose:
            print(f"📦 {forge.name:30} → {dst_mod.name}")
        
        try:
            if fnmatch.fnmatch(forge.name.lower(), "*.zip"):
                with zipfile.ZipFile(forge) as zf:
                    zf.extractall(dst_mod)
            else:
                with tarfile.open(forge) as tf:
